...
import { FileText, Plus, CalendarDays, Pill, Stethoscope, Activity, ArrowLeft, ShieldCheck, Printer, Paperclip, XCircle, FileImage, FileVideo, FileType, Edit3 } from 'lucide-react'; // Removed Trash2
...
const NewEventFormComponent: React.FC<NewEventFormProps> = ({ petId, onSave, onClose }) => {
    ...
    // const [surgeryCost, setSurgeryCost] = useState<string>(''); // Costo eliminado
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (tipoEvento === TipoEventoHistorial.ENFERMEDAD_REGISTRADA) {
            ...
            recordPetDisease(petId, selectedDiseaseId, fecha, diseaseNotes || "Sin observaciones específicas.", eventDateTime);
            ...
            // const costValue = surgeryCost !== '' ? parseFloat(surgeryCost) : undefined; // Costo eliminado
            // if (surgeryCost !== '' && (isNaN(costValue!) || costValue! < 0)) { // Costo eliminado
            //     alert("El costo de la cirugía debe ser un número válido o estar vacío."); return; // Costo eliminado
            // } // Costo eliminado
            recordPetSurgery(petId, selectedSurgeryId, fecha, surgeryNotes || "Sin observaciones específicas.", undefined, eventDateTime); // Costo eliminado, se pasa undefined
        }
        ...
        if (descripcion.trim() || attachments.length > 0) {
            ...
            newEvent = addMedicalHistoryEvent(
                petId, 
                tipoEvento, 
                eventDescription, 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <FormField 
                ...
                onChange={(e) => {
                    ...
                    setSelectedSurgeryId(''); setSurgeryNotes(''); // setSurgeryCost(''); // Costo eliminado
            ...
            {tipoEvento === TipoEventoHistorial.CIRUGIA && (
                 <>
                    ...
                    {/* <FormField label="Costo Final (Opcional)" name="surgeryCost" type="number" value={surgeryCost} onChange={(e) => setSurgeryCost(e.target.value)} min="0" step="0.01"/> Costo eliminado */}
                </>
            )}
...
export const MedicalHistoryPage: React.FC = () => {
  ...
  const handleEditEventSave = (updatedData: Partial<Pick<HistorialMedico, 'fecha' | 'descripcion' | 'attachments'>>) => {
    if (editingEvent) {
      updateMedicalHistoryEvent(editingEvent.id_evento, updatedData);
    }
  ...
  // const handleDeleteEvent = (eventId: string) => { // Function removed
  //   if (window.confirm("¿Está seguro desea borrar esta entrada?")) {
  //       deleteMedicalHistoryEvent(eventId);
  //   }
  // };
      ...
      {historyEvents.length > 0 ? (
        <div className="space-y-4">
          {historyEvents.map(event => (
            <div key={event.id_evento} className="bg-white shadow-lg rounded-lg p-4 sm:p-5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2">
                <div className="flex items-center">
                  {getEventIcon(event.tipo_evento)}
                  <h3 className="text-lg font-semibold text-primary-700">{event.tipo_evento}</h3>
                </div>
                <div className="flex items-center gap-1 sm:gap-2 mt-1 sm:mt-0">
                    ...
                    <Button 
                        ...
                        className="text-error-500 hover:bg-error-50 p-1"
                    >
                        <Trash2 size={16} />
                    </Button>
                    */}
              ...
              <p className="mt-1 text-secondary-700 whitespace-pre-wrap text-sm">{event.descripcion}</p>
              {event.referencia_id && (
                <p className="mt-2 text-xs text-secondary-500 italic border-t border-secondary-200 pt-2">{getReferenceDetails(event)}</p>
              )}
                            ...
                            <li key={att.id} className="text-xs">
                                ...
                                >
                                    ...
                                    {att.type.startsWith('image/') && (
                                      <img src={att.dataUrl} alt={`Miniatura de ${att.name}`} className="h-6 w-auto ml-2 border border-secondary-200 rounded object-cover"/>
                                    )}
                                </a>
                            </li>
                        ))}
