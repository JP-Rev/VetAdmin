...
const ProductFormComponent: React.FC<ProductFormProps> = ({ initialData, onSave, onClose }) => {
  ...
  const [formData, setFormData] = useState<ProductoForm>(
    initialData
      ...
      : { nombre: '', stock: 0, precio: 0, categoria: '' }
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormField label="Categoría" name="categoria" value={formData.categoria} onChange={handleChange} error={errors.categoria} required />
      <div className="grid grid-cols-2 gap-4">
        ...
        <FormField label="Precio Unitario ($)" name="precio" type="number" value={String(formData.precio)} onChange={handleChange} error={errors.precio} required />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow-lg rounded-lg overflow-x-auto">
          ...
          <tbody className="bg-white divide-y divide-secondary-200">
            {filteredProducts.length > 0 ? filteredProducts.map(product => {
              ...
              <tr key={product.id_producto} className="hover:bg-secondary-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">{product.categoria}</td>
       ...
       {filteredProducts.some(p => p.stock > lowStockThreshold && p.stock <= warningStockThreshold) && 
        !filteredProducts.some(p => p.stock <= lowStockThreshold) && // Show only if no critical alerts
