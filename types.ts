...
export enum EstadoVenta { // Renamed from EstadoOrden
  PENDIENTE = 'Pendiente',
  PAGADA = 'Pagada',
  CANCELADA = 'Cancelada',
}
...
export interface VentaProducto { // Renamed from OrdenProducto
  producto_id: string; 
  cantidad: number;
...
export interface Venta { // Renamed from Orden
  id_venta: string; // Renamed from id_orden
  fecha: string; 
  cliente_id: string; 
  mascota_id?: string; 
  productos: VentaProducto[]; // Renamed from OrdenProducto
  total: number;
  estado: EstadoVenta; // Renamed from EstadoOrden
  lastModified: number;
}

export interface Pago {
  id_pago: string;
  venta_id: string; // Renamed from orden_id
  monto: number;
  fecha: string; 
  metodo: MetodoPago;
  lastModified: number;
...
export interface ConsultationFormState {
  ...
  attachments: AttachmentFile[]; // Added for potential future use in ConsultationForm
}
...
export type VentaFormValues = { // Renamed from OrderFormValues
  cliente_id: string;
  mascota_id?: string;
