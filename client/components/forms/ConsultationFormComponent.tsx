      ...
      <div className="p-4 border border-secondary-200 rounded-md bg-secondary-50">
        ...
        {formState.diseases.length > 0 && (
            ...
            {formState.diseases.map(d => {
                ...
                return <li key={d.id} className="flex justify-between items-center p-1.5 bg-surface rounded shadow-sm"><span>{diseaseDetail?.nombre}: {d.notes.substring(0,30)}...</span><Button size="sm" variant="ghost" onClick={() => handleRemoveDisease(d.id)} className="text-error-500 p-1"><Trash2 size={14}/></Button></li>
            })}
            </ul>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
         ...
         {formState.surgeries.length > 0 && (
            ...
            {formState.surgeries.map(s => {
                ...
                return <li key={s.id} className="flex justify-between items-center p-1.5 bg-surface rounded shadow-sm"><span>{surgeryDetail?.tipo}: {s.notes.substring(0,25)}... {s.cost ? `($${s.cost})` : ''}</span><Button size="sm" variant="ghost" onClick={() => handleRemoveSurgery(s.id)} className="text-error-500 p-1"><Trash2 size={14}/></Button></li>
            })}
            </ul>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
         ...
         {formState.vaccinations.length > 0 && (
            ...
            {formState.vaccinations.map(v => 
                <li key={v.id} className="flex justify-between items-center p-1.5 bg-surface rounded shadow-sm"><span>{v.vaccineName}: {v.notes.substring(0,30)}...</span><Button size="sm" variant="ghost" onClick={() => handleRemoveVaccination(v.id)} className="text-error-500 p-1"><Trash2 size={14}/></Button></li>
            )}
            </ul>
        )}
      </div>
