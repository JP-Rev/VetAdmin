...
export const ConsultationFormComponent: React.FC<ConsultationFormComponentProps> = ({ appointment, onSave, onClose }) => {
  ...
  const [formState, setFormState] = useState<ConsultationFormState>({
    mainDescription: '',
    eventDate: appointment.fecha, // Default to appointment date
    diseases: [],
    surgeries: [],
    vaccinations: [],
    attachments: [], // Added missing attachments property
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const eventDateTime = `${formState.eventDate} ${appointment.hora}`; // Combine form date with appointment time

    // 1. Main consultation event
    // The main description will also carry any attachments added directly to this consultation form.
    // If no main description, but attachments exist, create a generic event description.
    ...
    if (mainEventDescription) { // Only create main event if there's a description or attachments
      addMedicalHistoryEvent(
        appointment.mascota_id,
        TipoEventoHistorial.CONSULTA,
        mainEventDescription,
        appointment.id_turno, // Reference the appointment
        eventDateTime,
        formState.attachments // Pass attachments here
      );
    }
    ...
    formState.diseases.forEach(d => {
      // Attachments for diseases are handled separately if needed by modifying recordPetDisease or via generic event
    ...
    formState.vaccinations.forEach(v => {
      ...
      addMedicalHistoryEvent(
        ...
        undefined, 
        eventDateTime
        // No direct attachments here for vaccinations unless addMedicalHistoryEvent is extended
      );
    });
  ...
  // Note: Attachment handling directly within this ConsultationFormComponent is simplified.
  // The `formState.attachments` would typically be populated by a file input within this component.
  // For this fix, we are only addressing the type error by initializing `attachments: []`.
  // A full implementation would require a file input and logic to populate `formState.attachments`.
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-4 border border-secondary-200 rounded-md bg-secondary-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <FormField label="Enfermedad" name="tempDiseaseId" as="select" value={tempDisease.diseaseId} onChange={(e) => setTempDisease(p => ({...p, diseaseId: e.target.value}))} options={diseases.map(d => ({ value: d.id_enfermedad, label: d.nombre }))} placeholder="Seleccionar enfermedad" className="mb-0 md:col-span-1"/>
      ...
      <div className="p-4 border border-secondary-200 rounded-md bg-secondary-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            <FormField label="Cirugía" name="tempSurgeryId" as="select" value={tempSurgery.surgeryId} onChange={(e) => setTempSurgery(p => ({...p, surgeryId: e.target.value}))} options={surgeries.map(s => ({ value: s.id_cirugia, label: s.tipo }))} placeholder="Seleccionar cirugía" className="mb-0 md:col-span-1"/>
