  ...
  return (
    <div className="p-4 font-sans">
      ...
      <section className="mb-4 p-3 border border-gray-300 rounded">
        <h3 className="text-lg font-semibold mb-1">Datos de la Mascota</h3>
      ...
      <section>
        <h3 className="text-lg font-semibold mb-2">Eventos Médicos</h3>
        {historyEvents.length > 0 ? (
          <div className="space-y-3">
            {historyEvents.map(event => (
              <div key={event.id_evento} className="pb-2 mb-2 border-b border-gray-200 last:border-b-0">
                ...
                <p className="whitespace-pre-wrap"><strong>Descripción:</strong> {event.descripcion}</p>
                {event.referencia_id && (
                  <p className="text-xs mt-1"><em>Detalles Adicionales: {getReferenceDetailsForPrint(event)}</em></p>
                )}
                {event.attachments && event.attachments.length > 0 && (
                  <div className="mt-1">
                    <p className="text-xs"><em>Archivos Adjuntos: {event.attachments.map(att => att.name).join(', ')}</em></p>
                  </div>
                )}
              </div>
            ))}
      ...
      <footer className="mt-6 pt-3 border-t border-gray-300 text-center text-xs">
